...
func createConstantVarying() lanes.Varying[int] {
	...
	return lanes.Varying[int](42)  // All lanes get same value (broadcast)
}
...
func arrayToVarying() lanes.Varying[int] {
	...
	return lanes.From(data[:])  // Convert array slice to varying
}
...
func illegalFunction() lanes.Varying[int] {
	...
	return lanes.Varying[int](999)  // broadcast 999 to all lanes
}
...
func uniformToVarying(base int) lanes.Varying[int] {
	...
	// Create varying from uniform input (broadcast)
...
func complexProcessing() lanes.Varying[float64] {
	// Create multiple varying values (broadcasts)
