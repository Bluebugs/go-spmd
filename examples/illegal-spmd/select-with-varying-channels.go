...
func main() {
	...
	var ch lanes.Varying[chan int]  // ERROR: varying chan T syntax not supported
...
type ChannelStruct struct {
	varyingCh lanes.Varying[chan int]  // ERROR: varying chan T not supported
}

// ILLEGAL: Map with varying channel types
func mapChannelExample() {
	var chMap map[string]lanes.Varying[chan int]  // ERROR: varying chan T not supported
	_ = chMap
}

// ILLEGAL: Interface with varying channel types
type ChannelInterface interface {
	GetChannel() lanes.Varying[chan int]  // ERROR: varying chan T not supported
}

// ILLEGAL: Function parameters with varying channel types
func sendToVaryingChannel(ch lanes.Varying[chan int], data int) {  // ERROR: varying chan T parameter
	ch <- data
}

func receiveFromVaryingChannel(ch lanes.Varying[chan int]) int {  // ERROR: varying chan T parameter
	return <-ch
}
