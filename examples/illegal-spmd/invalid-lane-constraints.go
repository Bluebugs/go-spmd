...
func main() {
	...
	go for i := range[4] 100 {  // Processing in groups of 4
		constrained[i] = i  // ERROR: assigning to Varying[int, 8] in range[4] context
	}

	// ILLEGAL: Function parameter constraint mismatch
	processGrouped4(constrained)  // ERROR: cannot pass Varying[int, 8] to Varying[int, 4] parameter

	_, _, _ = data1, data2, data3
}
...
func runtimeConstraint() {
	...
	for constraint := 4; constraint <= 16; constraint *= 2 {
		// var data lanes.Varying[int, constraint]  // ERROR: non-constant constraint
		_ = constraint
	}
}
...
func varyingConstraintValue() {
	go for i := range 4 {
		...
		// var data lanes.Varying[int, lane_constraint]  // ERROR: varying constraint
		_ = lane_constraint
	}
}
