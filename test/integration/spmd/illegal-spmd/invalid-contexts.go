...
var global_varying lanes.Varying[int] = 42  // ERROR: varying types not allowed at package level
...
func invalidPointers() {
	...
	var basePtr *int
	var varyingOffset lanes.Varying[int] = 5
...
func invalidTypeSwitch() {
	var varying_interface lanes.Varying[interface{}] = lanes.Varying[interface{}](42)
...
func validRecover() {
	go for i := range 3 {
		defer func() {
			if r := recover(); r != nil {
				var varying_r lanes.Varying[interface{}] = r  // LEGAL: recover() in varying context
...
func processVaryingInterface(x lanes.Varying[interface{}]) {
	// Implementation
}

// ILLEGAL: Varying slice/array bounds
func invalidBounds() {
	var size lanes.Varying[int] = 10
	...
	var start lanes.Varying[int] = 2
	var end lanes.Varying[int] = 7
...
func invalidReduce() {
	...
	sum := reduce.Add(uniform_data[0])  // ERROR: int, not lanes.Varying[int]
