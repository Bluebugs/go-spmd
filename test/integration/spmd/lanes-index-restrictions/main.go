...
func spmdWithLanesIndex(data lanes.Varying[int]) lanes.Varying[int] {
...
// func nonSPMDWithLanesIndex() lanes.Varying[int] {
...
func nestedSPMDFunction(base lanes.Varying[int], multiplier lanes.Varying[int]) lanes.Varying[int] {
...
func demonstrateErrorCases() {
	...
	// func errorCase1() lanes.Varying[int] {
	//     return lanes.Varying[int](lanes.Index())  // ERROR: no varying params, no go for
	// }
	...
	// func errorCase3() lanes.Varying[int] {
	//     return lanes.Varying[int](lanes.Index())  // ERROR: still illegal even if called from go for
	// }
	//
	// func caller() {
	//     go for i := range 4 {
...
func processBatch(items lanes.Varying[int], offset lanes.Varying[int]) lanes.Varying[int] {
