...
// output sums are lane-count-invariant (same result on 4-wide WASM f32,
// 8-wide AVX2 f32, etc.).
package main
...
func testSqrt() float32 {
	// Perfect squares 1..16^2. sqrt sum = 1+2+...+16 = 136.
	data := []float32{1, 4, 9, 16, 25, 36, 49, 64, 81, 100, 121, 144, 169, 196, 225, 256}
...
func testFloor() float32 {
	// Fractional x.5 values. Floor(x.5) = x. Sum = 1+2+...+16 = 136.
	data := []float32{1.5, 2.5, 3.5, 4.5, 5.5, 6.5, 7.5, 8.5, 9.5, 10.5, 11.5, 12.5, 13.5, 14.5, 15.5, 16.5}
	var acc lanes.Varying[float32]
	go for i, x := range data {
		_ = i
		acc += lanes.Floor(x)
...
func testFMA() float32 {
	...
	go for i, a := range dataA {
		_ = i
		acc += lanes.FMA(a, float32(2), float32(0))
	}
	return reduce.Add(acc)
}

func testComposed() float32 {
